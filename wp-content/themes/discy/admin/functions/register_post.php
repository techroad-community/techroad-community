		...
		if ( ! empty( $match ) ) {
			...
			if ( in_array( $op, array( 'is', 'not' ) ) ) {
				...
				if (isset($field_values[$id][$val])) {
					$bool = (isset($field_values[$id][$val]["value"]) && $field_values[$id][$val]["value"] == $val);
				}else {
...
/* Style taxonomy */
discy_term_options(array('taxonomy' => 'category'));
discy_term_options(array('taxonomy' => 'question-category'));
function discy_term_options( $args ) {
	add_action( $args['taxonomy'] .'_add_form_fields', 'discy_term_add_form_fields' );
	add_action( $args['taxonomy'] .'_edit_form_fields', 'discy_term_edit_form_fields', 10, 2 );
	add_action( 'edited_'. $args['taxonomy'], 'discy_save_term', 10, 2 );
	add_action( 'create_'. $args['taxonomy'], 'discy_save_term', 10, 2 );
}
function discy_term_edit_form_fields( $tag ) {?>
	<tr class="form-terms">
		<th colspan="2" scope="row" valign="top">
			<div class="discy_framework">
				<?php discy_admin_fields_class::discy_admin_fields("term_edit",discy_terms,"terms",$tag->term_id,discy_admin_terms($tag->taxonomy));?>
			</div>
		</th>
	</tr>
	<?php
...
function discy_save_term( $term_id ) {
	...
	$options = discy_admin_terms($term->taxonomy);
