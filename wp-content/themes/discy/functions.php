...
if (!function_exists('discy_scripts_styles')) {
	function discy_scripts_styles() {
		...
		if (!empty($action_background) && ((is_user_logged_in() && ($action_logged == "logged" || $action_logged == "both")) || (!is_user_logged_in() && ($action_logged == "unlogged" || $action_logged == "both")))) {
	...
	function discy_head() {
		...
		if ($seo_active == "on") {
		    ...
		    if (!is_home() && !is_front_page() && (is_single() || is_page())) {
		    	...
		    	if ($last_og_image != "") {
		    		echo '<meta property="og:image" content="' . $last_og_image . '">' . "\n";
		    	}
...
if ( ! function_exists( 'discy_meta' ) ) {
	function discy_meta($date = "",$category = "",$comment = "",$asked = "",$icons = "",$views = "",$post_id = 0) {
		...
		if ( $date == "on" ) {
			...
			$posted_on = $data_string.'<span class="date-separator">:</span> '.$time_string;
